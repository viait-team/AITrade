const levelColors = {
    0: "#1f77b4", // Root: Blue
    1: "#2ca02c", // Derived: Green
    2: "#ff7f0e", // Composite: Orange
    3: "#d62728", // Valuation: Red
    4: "#7f7f7f"  // Output: Gray
};

const nodes = [
    { id: "P (Observed Market Price)", level: 0 },
    { id: "E (Reported Earnings)", level: 0 },
    { id: "PE (Price/Earnings Ratio)", level: 1 },
    { id: "g (Projected Growth Rate)", level: 1 },
    { id: "g′ (TTM Growth Rate)", level: 1 },
    { id: "Cumulative Compound Earnings", level: 2 },
    { id: "PEG (PE to Growth)", level: 2 },
    { id: "Fair Value (from fundamentals)", level: 3 },
    { id: "Break-even Threshold", level: 3 },
    { id: "Years (Time Horizon)", level: 4 }
];
...
function drawChart() {
    ...
    // Clear previous contents to prevent duplication on redraw
    svg.selectAll('*').remove();

    // 2. GET THE CONTAINER'S CURRENT DIMENSIONS
    // We get the width from the container's bounding box.
...
// This is more efficient than window.addEventListener('resize', ...).
